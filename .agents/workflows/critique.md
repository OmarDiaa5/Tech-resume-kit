...
8b. **Project Hook Verification:** If the CL cites named projects, companies, programs, or technologies, web-search to verify accuracy. Flag factual errors as Tier 1 fixes.
...
11. **Evidence Checkpoint (Lightweight)**
...
### >>>>>> ANTIGRAVITY OPTIMIZATION: CRITIQUE ARTIFACT <<<<<<
**Do NOT dump the full multi-page 8-section critique directly in the chat thread.** 
Write the full 8-section critique report directly into the persistent file `output/<FolderName>/critique_<name>.md` and render it as a **Markdown Artifact** for the user.
In the active chat, present only a high-level **Critique Summary** containing:
- The Eight-Dimension Score Table
- Actions list of **Tier 1 (fatal/serious) fixes**
- Interview Likelihood probability
This prevents chat history bloat and saves extensive token quota.

### >>>>>> MANDATORY STOP <<<<<<
Present the Critique Summary and point the user to the full Critique Artifact. Wait for the user's explicit text response before continuing.
